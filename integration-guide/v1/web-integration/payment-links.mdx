...
- **Match payments to the link on `link_id`.** A buyer may try more than once before a payment goes through. The payment webhooks — [`PAYMENT_SUCCESSFUL`](/api-reference/v1/webhooks/payment-successful) when a payment goes through — carry that payment's own `payment_id` and the link's `link_id`.
...
Where each try is a payment of its own, a try that fails sends no webhook: the link stays open and the buyer can pay again through it. If the link closes without being paid — it expires, you deactivate it, or 20 tries have failed — one [`PAYMENT_FAILED`](/api-reference/v1/webhooks/payment-failed) arrives for the link itself, with `payment_id` equal to `link_id`. There is nothing to refund on it, and it is not a payment the payments API returns: look the link up by `link_id` with [Retrieve Payment Link](/api-reference/v1/payment-link/retrieve).
