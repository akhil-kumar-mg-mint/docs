...
A payment link and the payment made through it share one ID. Create Payment Link returns it as `link_id`, and as `payment_id` with the same value; the payment webhooks for the link's payment carry it as both. Use it to manage the link ([Retrieve Payment Link](/api-reference/v1/payment-link/retrieve), [Deactivate Link](/api-reference/v1/payment-link/deactivate)) and to refund and reconcile the payment ([Create Refund](/api-reference/v1/refunds/create)).
