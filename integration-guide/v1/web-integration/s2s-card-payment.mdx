...
1. **Automatic Save (During Payment)**: Add `"save_card": true` in the `card_details` object when initiating a payment. The card will be tokenized automatically upon successful payment.
2. **Manual Save (Dedicated API)**: Call the [Save Card API](/api-reference/v1/card-tokens/save) to tokenize card details independently of a payment.
...
Use the dedicated [Save Card API](/api-reference/v1/card-tokens/save) to tokenize cards manually:
...
- **[Save Card](/api-reference/v1/card-tokens/save)**: Tokenize card details without processing payment
- **[List Saved Cards](/api-reference/v1/card-tokens/list)**: Retrieve customer's saved cards
- **[Delete Saved Card](/api-reference/v1/card-tokens/delete)**: Remove saved card tokens
...
<Note>
  For detailed parameters and response formats, see the [Get Order API](/api-reference/v1/order/get)
</Note>
...
- [Payment Successful](/api-reference/v1/webhooks/payment-successful)
- [Payment Failed](/api-reference/v1/webhooks/payment-failed)
...
For additional support, consult our [API Reference](/api-reference/v1/) or contact our integration team.