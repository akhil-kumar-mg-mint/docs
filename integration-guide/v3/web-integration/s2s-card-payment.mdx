...
<CardGroup cols={3}>
  ...
  <Card title="Card Tokenization" icon="credit-card" href="#saved-card-tokens">
    Save cards during payment for faster repeat transactions without storing sensitive data
  </Card>
...
EximPe's card tokenization system allows you to securely save card details for future payments.

### Automatic Save During Payment

To save a card while processing a payment, include the `save_card` parameter in your `card_details` object. The card will be tokenized automatically upon successful payment.
...
- **[List Saved Cards](/api-reference/v3/card-tokens/list)**: Retrieve customer's saved cards
- **[Delete Saved Card](/api-reference/v3/card-tokens/delete)**: Remove saved card tokens
...
For additional support, consult our [API Reference](/api-reference/v3/) or contact our integration team. 