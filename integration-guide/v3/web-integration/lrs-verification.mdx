...
flowchart TD
    ...
    B["<code>POST /pg/lrs/quote</code><br/>Tells you the <b>TCS</b> to collect, plus our fee<br/>and the GST on it, added to the amount to be settled"]
    C["Buyer transfers that amount<br/>into the virtual account"]
    D["Webhook <code>LRS_VERIFICATION_NEEDED</code><br/>We list what the payment still owes"]
    E["<code>GET /pg/lrs/payments/:id/verification</code><br/>The full requirement set, with labels<br/>you can show the buyer"]
    F["<code>POST /pg/lrs/payments/:id/verification</code><br/>Send fields and documents"]
    G{"Anything<br/>outstanding?"}
    H{"Gateway<br/>verdict"}
    I["Settled to you"]

    A --> B --> C --> D --> E --> F --> G
    G -- "yes" --> F
    G -- "no" --> H
    H -- "<code>action_required</code>" --> F
    H -- "<code>under_review</code>" --> I
...
The first two calls happen before any money moves. The last two repeat until nothing is outstanding — partial sends merge, so send each thing as you get it rather than waiting for the full set.

### When it does not go straight through

| What happened | What you see | What to do |
| :--- | :--- | :--- |
| The PAN will not verify | `verified: false` with a `reason` | Resolve it with the buyer **before** you quote. Nothing downstream works until it passes. |
...
| Something is still missing | `missing_fields` / `missing_documents` on the response | Send just those. What you sent before is kept. |
| A complete set was refused | `handed_off: true` with `status: "action_required"` and a `reason` | Fix what `reason` names — usually the PAN or the sender match — and send again |
...
You get the whole requirement set — what is already satisfied and what is still owed — plus the `label` for each code. Those labels are written to be shown to a buyer, including the exact wording they must be shown to affirm `buyer_declaration`.
