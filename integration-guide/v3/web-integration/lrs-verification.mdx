...
flowchart TD
    ...
    E["<code>POST /pg/lrs/payments/:id/verification</code><br/>Send fields, and documents as refs"]
    ...
    U["<code>POST /pg/lrs/documents</code><br/>Upload each document as it arrives,<br/>keep the <code>ref</code> it returns"]
...
Two calls, and they are decoupled. Upload each document when you get it and keep the `ref` it returns; then send the fields and those refs together as JSON.
...
<Warning>
  **`ACTION_REQUIRED` never means your details were discarded.** Everything you sent is kept. `GET /pg/lrs/payments/:payment_id/verification/` names what is still outstanding; a failed check also arrives as an `LRS_VERIFICATION_NEEDED` event with a sentence per item.
</Warning>
