...
Every settlement blocker comes from doing one of these out of order. The two that cost the most are quoting before the PAN is verified, and asking the buyer for the amount to be settled instead of the amount to be collected.
...
%%{init: {
  ...
  "flowchart": { "curve": "basis", "nodeSpacing": 40, "rankSpacing": 44 }
}}%%
flowchart TD
    A["1 · Verify the buyer's PAN"]
    B["2 · Quote the tax"]
    C["3 · Ask for the<br/><b>amount to be collected</b>"]
    D["4 · Buyer transfers in"]
    E["5 · LRS_VERIFICATION_NEEDED<br/>names what is missing"]
    F["6 · Send fields and documents"]
...
Steps 1 to 3 happen before any money moves. Steps 4 to 6 repeat until nothing is outstanding — partial sends merge, so send each thing as you get it.
...
Steps 1 and 2 above are not part of the verification loop, and both are easy to skip.

- **[Verify PAN](/api-reference/v3/lrs/verify-pan)** — do it before you ask anyone for documents.
- **[Quote LRS Amount](/api-reference/v3/lrs/quote)** — returns `total_payable_inr`, the figure to ask the buyer for.
