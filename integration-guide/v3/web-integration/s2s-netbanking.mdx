...
<Note>
  ...
  - `netbanking_details.bank_code`: 4-digit bank code (see [Supported Banks](#supported-banks))
...
After the customer returns from the bank portal, query the Order Status API to confirm the payment outcome:

```bash
GET /pg/orders/{order_id}/status/
X-Client-ID: YOUR_CLIENT_ID
X-Client-Secret: YOUR_CLIENT_SECRET
...
| `FAILED` | Payment failed or was rejected by the bank |
| `CANCELLED` | Payment was cancelled by the customer |
| `EXPIRED` | Payment session expired |

## Step 4: Server-to-Server Callback Response

EximPe sends a webhook notification to your configured callback URL when the payment status changes:
...
| Federal Bank | `3020` |
| RBL Bank | `3039` |
| Punjab National Bank | `3038` |
| UCO Bank | `3054` |
| South Indian Bank | `3042` |
| Karnataka Bank | `3030` |

<Note>
  Use bank code `3333` for testing in sandbox mode — it simulates a successful net banking payment without hitting a real bank.
</Note>
...
| `INVALID_BANK_CODE` | Unrecognised bank code | Use a valid code from the [Supported Banks](#supported-banks) table |
