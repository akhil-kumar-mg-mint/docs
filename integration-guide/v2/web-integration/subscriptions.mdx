...
<CardGroup cols={3}>
  ...
  <Card title="Adhoc Recurring Payment" icon="credit-card" href="#step-4-trigger-adhoc-recurring-payment">
    Trigger a recurring payment for ADHOC subscriptions
  </Card>
  <Card title="View Payments" icon="list" href="#step-5-view-subscription-payments">
    Fetch all payments associated with a subscription
  </Card>
  <Card title="Sandbox Testing" icon="flask" href="#step-6-sandbox-testing">
...
The integration consists of six steps:

1. **Create Subscription**
2. **Check Mandate Status**
3. **Modify or Cancel Mandate**
4. **Trigger Adhoc Recurring Payment**
5. **View Subscription Payments**
6. **Sandbox Testing**
...
## Step 4: Trigger Adhoc Recurring Payment

For **Cashfree ADHOC** subscriptions, recurring payments are not automatic — you must trigger each charge via the [**Trigger Recurring Payment**](https://docs.eximpe.com/api-reference/v2/subscriptions/recurring-payment) API. The subscription must have an active mandate before a payment can be triggered.

<Info>
This step applies only to **Cashfree ADHOC** subscriptions. For standard billing cycle subscriptions (MONTHLY, WEEKLY, DAILY), payments are processed automatically — see [Automatic Recurring Payments](#automatic-recurring-payments).
</Info>
...
2. **Asynchronous Processing** — Cashfree processes the payment (scheduled for T+1)
...
```json
{
  ...
  "data": {
    ...
    "status": "success",
    "message": "Cashfree ADHOC recurring payment initiated successfully"
  }
}
```
...
## Step 5: View Subscription Payments
...
The `payment_status` field accepts `SUCCESS` or `FAILED`. The `payment_id` should be a pending payment obtained from the [Fetch PG Payments](#51-fetch-pg-payments) response.
