...
4. **Simulate a Transfer (Sandbox)**: Test the end-to-end flow by simulating an incoming bank transfer to your VBA.

## Step 1: Create VBA

Create a Virtual Bank Account for the authenticated merchant. From v2.0.0 onwards, a merchant can have multiple **ACTIVE** VBAs simultaneously.
...
- **Amount lock** — restrict the VBA to accept transfers within a `min_amount`/`max_amount` range.

Both locks are persisted on the VBA and echoed back on every read. The remitter lock can also be updated after creation via the [Update VBA API](/api-reference/v2/vba/edit).
...
When a payment is received in a VBA account:
...
In the sandbox environment, you can simulate an incoming bank transfer to your VBA to test the end-to-end flow — including webhooks and payment reconciliation — without making a real bank transfer.

Pass the `virtual_account_id` along with simulated remitter details (amount, UTR, remitter name, account, and IFSC). The `utr` value must be unique across simulations.

<Warning>This endpoint is available only in the sandbox/staging environment, not in production.</Warning>

<Card title="📚 API Reference" href="/api-reference/v2/vba/simulate-transfer">
...
- **[Create VBA](/api-reference/v2/vba/create)**: Create a new VBA, optionally with remitter and/or amount locks.
- **[Get VBA Details](/api-reference/v2/vba/get)**: Fetch details of an existing VBA.
- **[Edit VBA](/api-reference/v2/vba/edit)**: Update name, email, phone, or remitter lock associated with the VBA.
...
- **[Simulate VBA Transfer](/api-reference/v2/vba/simulate-transfer)**: Simulate an incoming transfer to a VBA in sandbox.
