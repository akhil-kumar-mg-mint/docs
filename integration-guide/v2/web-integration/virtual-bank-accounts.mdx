...
Both locks are persisted on the VBA and echoed back on every read. Locks on a Cashfree VBA can also be updated after creation via the [Edit VBA API](/api-reference/v2/vba/edit).
...
- **[Edit VBA](/api-reference/v2/vba/edit)**: Update the name, email, phone, remitter lock, or amount lock on a Cashfree VBA.
