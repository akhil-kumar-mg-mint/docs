...
<Tabs>
  ...
  <Tab title="Data Object Fields">
    ...
    <ParamField path="data.link_id" type="string">
      ID of the payment link this payment was made through — the `link_id` that [Create Payment Link](/api-reference/v1/payment-link/create) returned, which is the same value as `payment_id`. `null` when the payment was not made through a payment link; the key is always present.
