...
Every payment made through the link arrives in payment webhooks with its own `payment_id` and the link's `link_id`. See [Tracking a payment link](/integration-guide/v1/web-integration/payment-links#tracking-a-payment-link).
