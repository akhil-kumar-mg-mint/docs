...
### 3. S2S Card Payment
Server-to-server card payment flow with 3D Secure authentication. For new cards, the system initiates 3D Secure verification and returns an ACS template that must be rendered on the client side to complete authentication. Card details are securely processed and can be tokenized using the [Create Token API](/api-reference/v1/card-tokens/save) after successful payment.
...
1. **New Card Details** (for first-time payments):
   ...
   - Use the [Create Token API](/api-reference/v1/card-tokens/save) after successful payment to tokenize for future use

2. **Saved Card Token** (for repeat payments):
   - Use `token`: Previously saved card token from the [Create Token API](/api-reference/v1/card-tokens/save)
...
- **Card Tokenization**: Use the [Create Token API](/api-reference/v1/card-tokens/save) after successful payment to securely tokenize cards for future use
...
- **Token Management**: Use the [Card Token APIs](/api-reference/v1/card-tokens/save) for managing saved cards
