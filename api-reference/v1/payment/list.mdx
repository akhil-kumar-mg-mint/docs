...
<Note>
  **Payment links.** Where each try at a payment link is a payment of its own, the list holds those payments but not the link itself. Use [List Payment Links](/api-reference/v1/payment-link/list) for the links.
</Note>
