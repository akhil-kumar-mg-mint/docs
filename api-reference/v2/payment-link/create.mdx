...
The request body follows the same structure as the [Create Order](/api-reference/v2/order/create) API, but it's specifically optimized for generating shareable links.
...
The response carries the `payment_link` to share and the link's `link_id`, which the other payment link endpoints take. `payment_id` is the same value, kept for compatibility.

Every payment made through the link arrives in payment webhooks with its own `payment_id` and the link's `link_id`. See [Tracking a payment link](/integration-guide/v2/web-integration/payment-links#tracking-a-payment-link).
