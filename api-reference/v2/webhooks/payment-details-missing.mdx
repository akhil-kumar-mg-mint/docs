...
This webhook notifies you which fields are missing for payment verification so you can supply them via the Partner API (order update or **upload-verification-details**) or dashboard. It is used for any payment with missing verification details, including VBA transfer and regular payment success flows.
...
<Tabs>
  <Tab title="Root Level Fields">
    ...
    <ParamField path="data" type="object" required>
      Event payload: order_uid, link_id, payment_id, message, details_missing
    </ParamField>
  </Tab>
  ...
  <Tab title="Data Object Fields">
    ...
    <ParamField path="data.link_id" type="string">
      ID of the payment link the payment was made through — the `link_id` that [Create Payment Link](/api-reference/v2/payment-link/create) returned. `null` when the payment was not made through a payment link; the key is always present.
