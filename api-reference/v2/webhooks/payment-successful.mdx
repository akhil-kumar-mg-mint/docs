...
4. **If `data.link_id` is not `null`**, the payment was made through that payment link — refund and reconcile it by `data.payment_id`. See [Tracking a payment link](/integration-guide/v2/web-integration/payment-links#tracking-a-payment-link).
5. **Note:** You may subsequently receive a **PAYMENT_DETAILS_MISSING** webhook if verification details are missing on the order (see [Payment Details Missing](/api-reference/v2/webhooks/payment-details-missing)).
