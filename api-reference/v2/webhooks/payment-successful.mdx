...
<Tabs>
  ...
  <Tab title="Data Object Fields">
    ...
    <ParamField path="data.link_id" type="string">
      ID of the payment link this payment was made through — the `link_id` that [Create Payment Link](/api-reference/v2/payment-link/create) returned, which is the same value as `payment_id`. `null` when the payment was not made through a payment link; the key is always present.
...
4. **If `data.link_id` is not `null`**, the payment was made through that payment link; `data.link_id` and `data.payment_id` are the same value.
