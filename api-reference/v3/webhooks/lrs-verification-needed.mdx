  ...
  "data": {
    ...
    "action_required_documents": [
      ...
      },
      {
        ...
        "label": "Student visa or University ID card",
        "reason": "REQUIRED",
...
<Tabs>
  ...
  <Tab title="Entries">
    ...
    <ParamField path="label" type="string">
      ...
      Always present on documents. On fields, present only where it says more than the
      code does: you get no `label` for `buyer_city`, and the full declaration text for
      `buyer_declaration`.
    </ParamField>

    <ParamField path="reason" type="string">
      Documents only. Branch on this, not on `message`.

      | Value | Meaning |
      |---|---|
      | `REQUIRED` | Owed outright for this purpose code |
      | `CONDITIONALLY_REQUIRED` | Owed because a condition now holds — `message` names it |
      | `OPTIONAL` | Accepted but not owed; listed until you send it |
    </ParamField>

    <ParamField path="message" type="string" required>
      The reason in a sentence, safe to render to a payer. On a merged alternative it
      names the two codes you may send; on a conditional it names the condition.
    </ParamField>
...
An optional item — the fee invoice on `S0305` — appears with `reason: "OPTIONAL"`
and stops appearing once you supply it.
...
<Steps>
  <Step title="Collect what the entries name">
    Show the payer each `label` where there is one, and each `message` for why it is needed. A code containing `/` is one obligation — ask for either half.
  </Step>
