...
<Note>
  **This event is Cashfree-specific.** A Cashfree payment missing order fields
  gets `PAYMENT_DETAILS_MISSING`. Every other gateway — HyperPG, PayU, ICICI
  (including a credit landing in a Virtual Bank Account), the sandbox simulator
  — reports the same gap through
  [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed)
  instead, in `action_required_fields`. If you integrate with more than one
  gateway, handle both events for the missing-order-fields case.
</Note>
...
**Category**: Payment  
**Description**: A Cashfree payment was recorded but one or more order fields required for payment verification are missing.

This webhook notifies you which fields that are missing for payment verification so you can supply them via the Partner API (order update or **upload-verification-details**) or dashboard. It is sent for Cashfree payments only — every other gateway reports the same gap through `VERIFICATION_NEEDED` (see the note above).
...
1. **The payment gateway is Cashfree** – every other gateway reports missing order fields through [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed) instead.
2. **Merchant has webhook URL** – Your merchant account (or parent, for sub-merchants) has an active API credential with a non-empty webhook URL.
3. **At least one detail is missing** – The payment’s order is missing one or more verification fields (see [details_missing](#details_missing-field-names) below). If all six fields are present on the order, the webhook is not sent — `VERIFICATION_NEEDED` covers anything still outstanding from there (an unresolved classification, or a gateway-specific document/settlement flag).
...
<Note>
  On [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed),
  the equivalent gap is reported the same way, as ordinary entries in
