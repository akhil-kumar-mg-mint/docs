...
This event fires **instead of** [`LRS_VERIFICATION_NEEDED`](/api-reference/v3/webhooks/lrs-verification-needed), and one common reason is that the payment could not be classified. Two facts decide that — the buyer type and the FEMA purpose code — and the event names them:
...
<ParamField path="data.classification.business_model" type="string">
  `B2C` or `B2B`, as resolved for this order.
</ParamField>

<ParamField path="data.classification.purpose_code" type="string">
  The FEMA purpose code the order settles under, or `null` if none has been declared.
</ParamField>

<ParamField path="data.classification.missing" type="array" required>
  Which of the two we could not resolve. Empty when both are known.

  `business_model` appears here when nothing has *declared* it — not when it is empty.
  It is never empty; it defaults to `B2C`. A default nobody stood behind is worth
  telling you about.
</ParamField>

Anything in `missing` also appears in this event's own missing list, because it
is a detail that is missing and that is the list you already read.
