...
4. **If `data.link_id` is not `null`**, the payment was made through that payment link — refund and reconcile it by `data.payment_id`. See [Tracking a payment link](/api-reference/v3/payment-link/create#tracking-a-payment-link).
5. **Note:** You may subsequently receive a **VERIFICATION_NEEDED** webhook if verification details are missing on the order (see [Verification Needed](/api-reference/v3/webhooks/verification-needed)). LRS payments receive [**LRS_VERIFICATION_NEEDED**](/api-reference/v3/webhooks/lrs-verification-needed) instead.
