...
2. **A B2B services submit passed.** A complete set was filed — through [Submit Verification Details](/api-reference/v3/b2b/submit-verification-details) or by the merchant in the dashboard — and the payment moved to Under Review to wait for an Ops decision. The synchronous response to a partner call reads `"status": "in_review"`. It fires again on each resubmit after a send-back.
3. **In sandbox**, when a simulated payment is moved to Under Review — either by supplying the last outstanding verification field, or by choosing that settlement status on the simulator page.

<Note>
  **On B2B services this event says "with Ops", not "accepted".** An Ops approval and an Ops rejection each raise nothing of their own — read them from [Get Verification Requirements](/api-reference/v3/b2b/verification-requirements). Only a send-back is announced, as [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed), and it reopens a payment this event had closed.
</Note>
