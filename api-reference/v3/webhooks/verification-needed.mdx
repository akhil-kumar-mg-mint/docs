...
4. **Ops sent a B2B services verification back for changes.** The payment returns to `ACTION_REQUIRED` and staging reopens. `message` carries the Ops comment rather than the standard sentence, and `action_required_fields` is often empty — Ops disagreed with what was sent rather than finding it short.
...
<Tabs>
  ...
  <Tab title="Data Object Fields">
    ...
    <ParamField path="data.action_required_fields" type="array of strings" required>
      Plain field codes that need your action. Empty when nothing is outstanding.
