...
<Note>
  `message` reads "Payment verification action required" on every one of these, including `IN_REVIEW`. It is a constant on this event type, not a description of the state — **branch on `verification_status`, never on `message`.**
</Note>
...
<Tabs>
  ...
  <Tab title="Data Object Fields">
    ...
    <ParamField path="data.message" type="string" required>
      A constant on this event type: `"Payment verification action required"`. Safe to log; not a value to branch on.
    </ParamField>
...
Supply these with [Upload Payment Verification Details](/api-reference/v3/payment/upload-verification-details).
