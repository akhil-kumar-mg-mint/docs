...
|---|---|
| `missing_fields[].code` | the key you put in `fields` |
| `missing_documents[].code` | what you name the file |

The response is the same shape again, so one call tells you what is left.

## The loop

<Steps>
  <Step title="An event arrives">
    [`LRS_VERIFICATION_NEEDED`](/api-reference/v3/webhooks/lrs-verification-needed) lists what the payment owes, and carries this endpoint's path in its `submit` block.
  </Step>
