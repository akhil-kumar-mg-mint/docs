...
An LRS payment answers a longer list than an ordinary one: the remitter's verified identity, the LRS declarations, and a set of documents that depends on the purpose code. This page is that list, and what happens once it is complete.
...
```json S0306 - Travel, other (today's body for S0303 / S0304 too — see note below)
{
  "fields": {
    "buyer_name": "Ravi Kumar",
    "buyer_address_line_1": "12 Brigade Road",
...
<Note>
  The three travel codes take an identical body **today**. They differ in what the remittance is *for* — other travel, medical treatment, pilgrimage — not in what you must collect.

  That's expected to change for two of them: `S0304` (medical treatment) and `S0303` (pilgrimage) are getting purpose-specific documents on top of the common travel set — a hospital or treatment document for medical, a pilgrimage-specific document for the other. Not finalized yet; we'll confirm the exact codes before either goes live for you. `S0306` stays as shown.
...
|---|:---:|:---:|:---:|:---:|:---:|
| Buyer and invoice fields | same | same | same | same | same |
| Remitter identity, declaration, `amount_to_be_settled` | same | same | same | same | same |
| Who the money is for, when the relation is not `SELF` | `student_name`, `student_dob`, `student_passport_number` | `student_name`, `student_dob` | `primary_person_name`, `primary_person_dob`, `primary_person_passport_number` | same as `S0306` | same as `S0306` |
| Required documents | `offer_letter`, `passport`, `student_id_or_visa` | `offer_letter`, `student_id` | `passenger_list`, `traveller_passports`, `traveller_visas`, `ticket`, `invoice` | same as `S0306`, **+ medical documents (TBD)** | same as `S0306`, **+ pilgrimage documents (TBD)** |
...
| `reason` | `REMAINING_AMOUNT` — covering an `amount_to_be_settled` shortfall. `TCS_PAYMENT` — paying the TCS separately. An audit label on the link. |
...
| `Not an amount; send a number like "500000.00".` | `amount_to_be_settled` that is not a decimal. |
