...
<ParamField path="refusal" type="object">
  ...
  Where `outstanding` says what the *checklist* still wants, this says what the *gateway* did. They answer different questions, which is exactly why a refusal usually arrives with `outstanding` empty.
  ...
  | `PAN_NAME_MISMATCH` | The PAN is verified, but its holder is not the buyer named on the order. |
  | `SENDER_NAME_MISMATCH` | The name on the credit that funded the payment is not the PAN holder's. |
...
| `Unknown document ref.` | The `ref` is not one of the uploads held for the merchant this payment settles to. |
