...
### B2B services is not on this page

The same envelope carries the non-LRS checklists, but what they ask for is different enough to be its own page. A B2B services payment has **no identity item at all** — no `remitter_pan`, no `iec` — and no amount to declare. See [Submit B2B Services Verification Details](/api-reference/v3/b2b/submit-verification-details).
...
<Note>
  **Not every checklist has a gateway handoff.** LRS and B2B services do — both answer `in_review` once a complete set is filed. A checklist with no gateway answers `complete` and stops there.
</Note>
