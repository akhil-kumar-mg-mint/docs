...
<Steps>
  <Step title="An event arrives">
    [`LRS_VERIFICATION_NEEDED`](/api-reference/v3/webhooks/lrs-verification-needed) lists what the payment owes — each with a label, a reason, and whether it can be waived — and carries this endpoint's path in its `submit` block.
  </Step>
...
<Note>
  **A `COMPUTED` field cannot be submitted.** `amount_to_be_settled` is worked out by
  EximPe; sending it returns `400`. It appears in `required_fields` so you know it
  exists and reaches the settlement file, but it is never something you owe.
</Note>
