...
### B2B services is the short one

The same envelope carries the non-LRS checklists. A complete **B2B services** body — `S0802`, `S0803`, `S1005`, `S1009`, `S1010`, `S1013`, `S1015`, `S1016`, `S1017`, `S1105`, `S1106` against `business_model: "B2B"` — is the nine common fields and one document, with **no identity item at all**: no `remitter_pan`, no `iec`, nothing verified against an external register.
...
A complete services set that files answers `status: "in_review"` — the payment is with EximPe Ops for review.

<Note>
  **No amount and no declaration key.** There is no `principal` and no `collected_tcs`: the amount that settles is the credit itself, read from the payment. TCS does not apply outside LRS. The declaration is recorded by the act of filing a complete set rather than sent as a field. All three are `400`s if you send them, like any unknown key.
</Note>

<Info>
  The whole flow — classify, read, upload, submit, then the post-settlement delivery-proof step — is in [B2B Services Verification](/integration-guide/v3/web-integration/b2b-services-verification).
</Info>
...
Filing also writes the staged values onto the order, and the order validates itself as it saves. Those refusals arrive in the same envelope, keyed by the same code names:

| Message | Cause |
|---|---|
| `Enter a valid Indian PIN code.` | `buyer_postal_code` is not a six-digit Indian PIN. The buyer on an import payment is the Indian remitter. |

A duplicate invoice number is the one refusal in this group whose `details` is a flat string rather than keyed by code:
