...
description: "Send a payment's verification details — fields and document refs — in one JSON call, keyed by the codes its checklist names."
...
<Warning>
  **A payment nothing has classified accepts only two keys.** Until the order has a business model and a purpose code no checklist claims the payment, so nothing else can be validated — sending anything more is refused rather than half-applied. Send the pair first, in one call, and the checklist appears:

  ```json
  { "fields": { "business_model": "B2C", "purpose_code": "S0305" } }
  ```

  They may travel in the same call as the rest, but they must travel **together**: the stored business model is a column default until classification lands, so a purpose code stamped against a defaulted model would be exactly the guess this flow exists to avoid. Once set, the pair is what the payment settles under and cannot be changed here.
</Warning>

<Note>
  **Neither is a checklist item, and an order can already be classified before you send anything.** They never appear in the `GET`'s `fields[]`, nor in [`LRS_VERIFICATION_NEEDED`](/api-reference/v3/webhooks/lrs-verification-needed) — they decide *which* checklist answers for the payment rather than belonging to one. You can state both when you create the order, and where your KYC names exactly one business model and one purpose code we resolve them for you, so many orders arrive here already classified.

  `GET` the payment before you send: a classification-stage answer means the pair is owed, a checklist means it is already set — and sending it again is a `400`, not a no-op.
</Note>
