...
```json Complete, but the gateway refused it
{
  "success": true,
  "message": "Verification requirements",
  "data": {
    ...
    "missing_fields": [],
    "missing_documents": [],
    "required_fields": ["… as above, all satisfied …"],
    "required_documents": ["… as above, all satisfied …"],
    "verification": {
      "handed_off": true,
    ...
    "verification_checked_at": "2026-08-17T06:25:20.866647"
  }
}
```
...
The second example is the one worth reading twice: **`status: "complete"` with both missing lists empty, and the payment still has not passed.** `verification` is where that shows — see [The gateway's last word](#the-gateways-last-word).
...
## The gateway's last word

Once a complete set has been handed to the verification gateway, the response
also carries what came back — the **same `verification` block** the
[submit response](/api-reference/v3/lrs/submit-verification-details#completion)
returns, so there is one shape to parse whichever way you arrived at it:
...
<Warning>
  **`status: "complete"` does not mean the payment passed.** It means you have
  sent everything the purpose code asks for. Whether the payment then cleared
  PAN verification, the sender match and the money check is what `verification`
  tells you — and the example above is a payment sitting in Action Required,
  ₹12,500 short, with an empty `missing_documents`. Reconcile on `verification`,
  not on `status`.
</Warning>

`verification_checked_at` is when that verdict was reached. It sits beside the
block rather than inside it, so the block itself stays identical to the submit
response's.

<Warning>
  **It is ISO 8601 in UTC with no offset** — `2026-08-17T06:25:20.866647`, not
  `…+00:00`. Parse it as UTC. A parser that treats a naive timestamp as local
  time will be wrong by your whole offset, which for IST is five and a half
  hours in the direction that makes a fresh verdict look stale.
</Warning>

<Note>
  **Absent until a complete set has been submitted** — and absent is not the same
  as approved. No `verification` key means the gateway has never been asked about
  this payment, so there is nothing to read into its silence.

  Nothing is re-run when you call this. A `GET` never verifies a PAN, posts to an
  LRS counter or moves a payment; it reports what the last attempt concluded.
</Note>
...
| `complete` | Nothing is outstanding **of what the purpose code asks for**. Read `verification` for whether the payment itself passed. |
