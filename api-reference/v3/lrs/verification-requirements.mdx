...
<Note>
  **Services asks for no identity.** Goods verifies the buyer's IEC and LRS verifies the remitter's PAN; services verifies neither. `buyer_name` is a plain value you send, checked against nothing external — so there is no step to clear before you can submit.
</Note>

A complete B2B-services requirement set is therefore the common nine fields and one document:
...
<Info>
  B2B services is walked end to end — including the post-settlement delivery-proof step that follows review — in [B2B Services Verification](/integration-guide/v3/web-integration/b2b-services-verification).
</Info>
