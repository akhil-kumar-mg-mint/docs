...
```json S0305, nothing supplied yet
{
  ...
  "data": {
    ...
    "fields": [
      ...
      {
        "code": "remitter_name",
        "message": "Remitter name (as per PAN)",
        "satisfied": false,
        "provided": false
      },
      {
      ...
      {
        "code": "primary_person_dob",
        "message": "Date of birth of the person the remittance is for",
        "satisfied": true,
        "provided": false,
        "condition": "PAYING_FOR_SOMEONE_ELSE"
      },
      {
        "code": "primary_person_passport_number",
        "message": "Passport number of the person the remittance is for",
        "satisfied": true,
        "provided": false,
        "condition": "PAYING_FOR_SOMEONE_ELSE"
      },
      ...
      {
        "code": "collected_tcs",
        "message": "TCS collected from the payer for this payment, in INR (\"0.00\" if none)",
        "satisfied": false,
        "provided": false
      }
    ],
...
| Status | When |
|---|---|
| `404` | No such payment for this merchant. The same answer whether it does not exist or belongs to someone else, so the endpoint never confirms another merchant's payment id. |
| `400` | The payment is classified, but no checklist covers that business model and purpose code — there is nothing to report. An **unclassified** payment is not this case: it answers `200`, asking for the two classifying facts. |
...
| Code | Asks for |
|---|---|
| `remitter_name` | Remitter name, as per PAN |
...
| `principal` | The amount to settle for this payment, in INR |
| `collected_tcs` | TCS collected from the payer for this payment, in INR — `"0.00"` if none |
| `primary_person_name` · `primary_person_dob` · `primary_person_passport_number` | Conditional: owed once `remitter_relation` is not `SELF` |

<Warning>
  **`collected_tcs` is collected, not computed for you.** It is what you took from the payer on this credit. Send `"0.00"` when nothing was collected — the key is still required, and omitting it leaves the payment outstanding. Quote the figure with [Quote LRS Amount](/api-reference/v3/lrs/quote) before you collect it.
</Warning>

Documents, per purpose code:

| Document code | `S0305` | `S1107` | `S0306` | `S0304` | `S0303` |
|---|:---:|:---:|:---:|:---:|:---:|
...
| `passenger_list` | — | — | required | required | required |
| `traveller_passports` | — | — | required | required | required |
| `traveller_visas` | — | — | required, waivable | required, waivable | required, waivable |
| `ticket` | — | — | required, waivable | required, waivable | required, waivable |
| `invoice` — invoice for the trip | — | — | required | required | required |
| `relationship_declaration` | conditional | conditional | conditional | conditional | conditional |
| Medical-treatment documents *(TBD)* | — | — | — | pending | — |
| Pilgrimage documents *(TBD)* | — | — | — | — | pending |
...
<Warning>
  **`S0304` and `S0303` are getting purpose-specific documents.** On top of the common travel set above, medical treatment (`S0304`) and pilgrimage (`S0303`) are each expected to ask for documents specific to that purpose — a hospital or treatment document for medical, a pilgrimage-specific document for the other. Not finalized: this table and the checklist endpoint will carry the real codes once they are. Read the checklist rather than hardcoding this table.
</Warning>
...
| `S0802` · `S0803` · `S1005` · `S1009` · `S1010` · `S1013` · `S1015` · `S1016` · `S1017` · `S1105` · `S1106` — services | `buyer_pan` | `invoice` |

## The requirement set can grow

Answering one requirement can reveal another. Declaring `remitter_relation` as anything other than `SELF` flips the three `primary_person_*` fields and the relationship self-declaration to `satisfied: false`, because they are only owed in that case.
