...
An entry in `documents` carries three more keys once a file has actually been sent under that code. They are absent while the slot is empty, so their presence is itself the signal that something is held:
...
<ParamField path="ref" type="string">
  The upload identifier [Upload Document](/api-reference/v3/lrs/upload-document) returned for the file currently held in this slot — the same `ref` you submitted. A replacement upload supersedes the older row, so this always names the current file.
</ParamField>
