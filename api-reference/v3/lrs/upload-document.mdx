...
<ParamField path="ref" type="string" required>
  ...
  **A PSP cannot carry one `ref` across two different sub-merchants.** The scope follows the settling account, so a file needed by two sub-merchants is uploaded once for each.
</ParamField>
