...
<Note>
  **There is no `action_required` on this flow.** A complete set either files — `in_review` — or is refused with a `400` naming what to fix. That fourth value belongs to the LRS gateway, which returns a verdict this one does not; do not write a branch for it.
</Note>
