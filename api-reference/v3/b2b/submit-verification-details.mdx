...
Send details keyed by the same codes that [Get Verification Requirements](/api-reference/v3/b2b/verification-requirements) and the [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed) webhook use. There is nothing to map between them — every one of those places names an item under `code`.
...
<Steps>
  <Step title="An event arrives">
    [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed) lists what the payment owes — each entry a code and a sentence.
  </Step>
...
<Warning>
  **The buyer is the Indian remitter, not your customer abroad.** `buyer_postal_code` is validated as a six-digit Indian PIN, so a foreign address is refused outright — `{"buyer_postal_code": "Enter a valid Indian PIN code."}`. This is the single most common way a first integration fails.
</Warning>

<Note>
  **No amount and no declaration key.** There is no `principal` and no `collected_tcs`: the amount that settles is the credit itself, read from the payment, and TCS does not apply outside LRS. The declaration is recorded by the act of filing a complete set rather than sent as a field. All three are `400`s if you send them.
</Note>
...
<Warning>
  **`action_required` never means your details were discarded.** Everything that reached us is held against the payment. You re-send the one thing that failed, not the whole set.
</Warning>
...
| `Enter a valid Indian PIN code.` | `buyer_postal_code` is not a six-digit Indian PIN. |

A duplicate invoice number is the one refusal here whose `details` is a flat string rather than keyed by code:
