...
<Note>
  **Re-uploading an invoice does not need a re-cite.** The citation names the `code`, and this flow has exactly one document type per code, so the payment's newest invoice is what is filed when the set completes. Upload a correction and it supersedes what you cited, without a second call here.
</Note>
