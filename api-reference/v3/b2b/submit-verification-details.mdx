...
<Note>
  **Branch on `code`, not on `message`.** `40359` is shared with the other verification flows; `message` names which one refused you, so a B2B services payment reads `Invalid B2B services request.` and an LRS payment reads something else. The code is the stable part.
</Note>
