...
<Note>
  **No amount and no declaration key.** The amount that settles is the credit itself, read from the payment, and no tax is collected on top — so there is no `amount_to_be_settled` to send. The declaration is recorded by the act of filing a complete set rather than sent as a field. Both are `400`s if you send them.
</Note>
...
| Message | Cause |
|---|---|
| `Not a requirement of this payment's checklist.` | The code is not one this set asks for — `hs_code`, `iec`, `amount_to_be_settled`, `invoice_amount` and `remitter_pan` all land here. |
