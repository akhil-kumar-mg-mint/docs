...
openapi: '/openapi/v3-b2b.json POST /pg/payments/{payment_id}/verification/'
---
...
Eleven fields and one document. This is the whole of it:
...
<Note>
  **Send the amount and the declaration; send no tax.** `invoice_amount` is a checklist field, in rupees, cross-checked against the credit — more than a paisa apart and the finalizing call is refused naming it. `declaration` is a boolean and has to be `true`. What settles is still the credit itself, and no tax is collected on top, so `amount_to_be_settled` stays a `400`.
</Note>
...
<Warning>
  **A citation is not what satisfies the item — the file is.** The `invoice` item reads as satisfied because an accepted invoice exists on the payment, not because you named a `ref`. So citing a ref again after Ops rejected that invoice leaves the item outstanding: upload a replacement, then cite the new `ref`. Check [Get Verification Requirements](/api-reference/v3/b2b/verification-requirements) rather than assuming a repeat citation cleared it.
</Warning>
...
| Message | Cause |
|---|---|
| `Not a requirement of this payment's checklist.` | The code is not one this set asks for — `hs_code`, `iec`, `amount_to_be_settled` and `remitter_pan` all land here. |
