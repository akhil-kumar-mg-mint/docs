...
Filing is also what writes the staged buyer and invoice fields onto the order. `invoice_number` is unique per merchant, so a collision surfaces here — under its own code, not `40359`:
