...
description: "Everything a B2B services payment is asked for — the eleven buyer and invoice fields, the commercial invoice, and which are already satisfied."
openapi: '/openapi/v3-b2b.json GET /pg/payments/{payment_id}/verification/'
---
...
<ParamField path="settlement_status" type="string" required>
  **The payment's own state — not a roll-up of the checklist.** This is the one thing an item list cannot tell you, and it is why this endpoint is worth calling when an event was missed: it says whether acting on the list is still possible at all.
  ...
  | `NOT_APPLICABLE` | **Ops rejected the verification**, or the payment carries no settlement workflow at all. Either way it will not settle and nothing you send reopens it. After a rejection a full refund is raised — follow that, not this endpoint. |

  The list is every value the key can hold, not every value you will see. A B2B-services payment you are verifying lives in the first three rows, and ends in `SETTLED` or `NOT_APPLICABLE`; the rest are here so an unexpected value is a value you can look up rather than a surprise.
...
<ParamField path="message" type="string" required>
  ...
  A refused filing is answered inline, in the `400`'s `details` keyed by the offending code. The same code also rides the [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed) event that refusal raises, so the two agree.
</ParamField>

<ParamField path="satisfied" type="boolean" required>
  Whether this item still blocks completion. `false` is what "missing" means.

  **It can go back to `false` on the `invoice`.** If Ops rejects the invoice during review, the item is owed again and the payment is sent back for it. Upload a replacement and cite the new `ref` — re-sending the rejected one changes nothing.
</ParamField>
...
| | Codes |
|---|---|
| Fields | `buyer_name`, `buyer_address_line_1`, `buyer_city`, `buyer_state`, `buyer_postal_code`, `buyer_email`, `buyer_phone`, `invoice_number`, `invoice_date`, `invoice_amount`, `declaration` |
| Documents | `invoice` |
...
<Warning>
  **`invoice_amount` is asked for, but it decides nothing.** Send it in rupees; it is cross-checked against the credit and refused if the two are more than a paisa apart. What settles is still the credit itself, and no tax is collected on top — `amount_to_be_settled` remains an unknown key here, and sending it is a `400`.
</Warning>
