...
```json
{
  ...
  "error": {
    ...
    "details": "Invalid LRS request. This payment has no verification checklist, so there is no set of document types to accept; send the order's business model and purpose code first."
  }
}
```
...
**The invoice: upload it again.** A payment has one commercial invoice, so a second `invoice` upload supersedes the first and the older `ref` stops resolving. You do not have to re-cite it: if you had already sent the earlier `ref` on the verification submit, the newest invoice is the one filed when the set completes.

**Delivery proofs accumulate.** Every `delivery_proof` upload adds a row, so you can collect them as they arrive — including before the payment settles. Nothing is filed until you [submit](/api-reference/v3/b2b/submit-settlement), and `refs` on that call is the whole set: an upload you would rather not file is simply one you leave out, and it is dropped.
