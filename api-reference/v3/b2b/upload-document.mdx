...
<Note>
  **The settling merchant is derived from the payment**, exactly as on the verification endpoints — the sub-merchant named on the order, not the credential that uploaded the file. `X-Merchant-ID` is neither required nor read on this route.
</Note>
