...
Once a B2B-services payment is **settled**, one more compliance step remains: evidence that the service was actually delivered. This endpoint reads what has been filed so far, and whether the set is locked.
...
An empty `documents` array means nothing has been filed yet — not that nothing is owed.

<ParamField path="state" type="string" required>
  `FRESH` (nothing filed yet) or `SUBMITTED` (filed and locked). Two values, not three — there is no draft on this surface, because files are held as uploads until one [submit](/api-reference/v3/b2b/submit-settlement) files them all. Read this before submitting: once it is `SUBMITTED`, a second submit is refused with `ERR_B2B_SETTLEMENT_002`.
</ParamField>

<ParamField path="is_settled" type="boolean" required>
  Whether the payment has settled. Documents can only be filed after it has; submitting earlier is refused with `ERR_B2B_SETTLEMENT_003`. This read is allowed either way.
</ParamField>
...
<ParamField path="required_doc_types" type="string[]" required>
  ...
  A `ref` uploaded under a type not listed here is refused by the submit with `ERR_B2B_SETTLEMENT_005`.
</ParamField>
...
<ParamField path="document_id" type="string" required>
  The filed document's own id. It is not the `ref` you submitted — that names the upload the document was made from, and stays valid on its own.
</ParamField>

<ParamField path="doc_type" type="string" required>
  The type the upload was stored under, carried onto the filed document — you never send a `doc_type` yourself. B2B services has exactly one, `delivery_proof`, and accepts **many files under it**. Goods is the opposite — one file per type — which is why the two flows do not share an endpoint.
</ParamField>
...
Only **active** documents, in the order they were filed. Nothing appears here until a [submit](/api-reference/v3/b2b/submit-settlement) files it: an uploaded `ref` you have not submitted yet is not part of this set, which is exactly what lets you hold it.
