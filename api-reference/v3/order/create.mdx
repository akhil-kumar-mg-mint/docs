...
| mop_type | string | Conditional | Method of payment. | Optional for `hosted_payment` (enum: `credit_card`, `debit_card`, `upi`, `net_banking`, `wallet`, `qr`) — pre-selects a method on the checkout page; must be `upi` for the S2S UPI Intent flow |
