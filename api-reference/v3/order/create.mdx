...
Both apply to either collection mode and resolve independently of each other:

- `business_model`: Whether the buyer is a business (`B2B`) or a consumer (`B2C`)
- `purpose_code`: The FEMA/FETERS purpose code this order settles under (e.g. `S0802`)

<Note>
  Omit either one and EximPe derives it from your KYC. A `purpose_code` is auto-filled only when your account has exactly one allowed code; where the KYC is ambiguous the field is left unset for later classification.
</Note>
...
| purpose_code | string | No | FEMA/FETERS purpose code this order settles under. | Must be active and enabled on your account (e.g. `S0101`, `S0802`); derived from your KYC when omitted |
...
A `purpose_code` you send is validated, not silently dropped:

| Condition | Code | Message |
|---|---|---|
| Unknown or inactive code | `40310` | Invalid Purpose code given. |
| Valid code, not enabled on your account | `40311` | Purpose code is not enabled for this merchant. |
| `business_model` outside `B2B` / `B2C` | `400` | `"business_model": ["\"X\" is not a valid choice."]` |
...
- Neither `business_model` nor `purpose_code` is echoed in the create response. Read them back with [Get Order Details](/api-reference/v3/order/get).
- Both are create-time only — `PATCH /pg/orders/{order_id}/` does not accept them.
- `business_model` is stored `NOT NULL` with a `B2C` default, so an order EximPe could not classify reads back as `B2C`, indistinguishable from one you sent as `B2C`. An unresolved `purpose_code`, being nullable, reads back as `null`.
