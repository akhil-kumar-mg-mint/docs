...
- **Match payments to the link on `link_id`.** A buyer may try more than once before a payment goes through. Every payment made through the link arrives in the payment webhooks — [`PAYMENT_SUCCESSFUL`](/api-reference/v3/webhooks/payment-successful) when it goes through, [`PAYMENT_FAILED`](/api-reference/v3/webhooks/payment-failed) when it does not — with its own `payment_id` and the link's `link_id`.
...
A link that closes without being paid also sends a [`PAYMENT_FAILED`](/api-reference/v3/webhooks/payment-failed) for the link itself, with `payment_id` equal to `link_id`. There is nothing to refund on it.
