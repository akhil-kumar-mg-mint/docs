...
A successful payment can still owe fields required for verification before it can be filed. Which webhook tells you depends on the gateway the payment arrived through — a Cashfree payment missing order fields gets `PAYMENT_DETAILS_MISSING`; every other gateway (including a credit landing in a Virtual Bank Account) reports the same gap through `VERIFICATION_NEEDED` instead. This endpoint is how you supply them programmatically either way, rather than editing the order in the dashboard.

## Verification flow

<Steps>
  <Step title="Payment recorded">
    A payment succeeds and is created against the order.
  </Step>
  <Step title="Missing fields reported">
    If verification fields are absent, [`PAYMENT_DETAILS_MISSING`](/api-reference/v3/webhooks/payment-details-missing) (Cashfree) or [`VERIFICATION_NEEDED`](/api-reference/v3/webhooks/verification-needed) (every other gateway) fires with the missing fields named in `details_missing` or `action_required_fields` respectively.
  </Step>
